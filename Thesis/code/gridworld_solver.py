...
class GridworldSolver(object):
    ...
    def Q_learning(self, start_state=None):
        ...
        episode = 10
        epsilon = 0.1 # weird, it does not work
        ...
        for i in range(episode):
            ...
            while not self.env.finished:
                ...
                # print(f'My next state is {next_state}\n')

                old_Q_value = Q[state.__str__()][action]
                next_max = np.max(Q[next_state.__str__()])

                new_Q_value = old_Q_value + 0.05 * (reward + 0.99 * next_max - old_Q_value)
                print(new_Q_value)
